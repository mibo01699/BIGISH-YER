...
* **Maximum Supply**: 300,000,000 YER (fixed via `YERTokenomicsCanonical.js`)
* **Allocations**: 10% Community (30M), 30% Ecosystem (90M), 60% A.E.C Reserve (180M)

## 🔄 2. Transaction Flow & Clearing Lifecycle (Sandbox/Testnet)

1. **Premium Calculation & DEX Conversion (Sandbox DEX Rate Pull)**:
   ...
   * AMAN queries the native `BIGISH-YER` **Sandbox** decentralized constant-product ($X \times Y = K$) exchange liquidity pool to pull the exact sub-unit price relation of Pi/USD.
   * The equivalent whole Pi amount is computed via fractionless multiplication and collected seamlessly through a **Supported Integration Status (Adapter-based)** boundary.
...
2. **Automated Loss Settlement & YER Disbursal**:
   ...
   * `BIGISH-YER` invokes the `AntiDoubleDippingEngine.js` and routes the liquidation payout natively to the beneficiary's wallet via the **Sandbox Pi / YER liquidity pool**.
   * This structure prevents rounding exploits and rules out cumulative truncation vulnerabilities entirely.

## 🔒 3. Concurrency Protection & Compliance (Sandbox)
...
* **Data Sovereignty**: Identity data is decoupled from public visibility, maintaining conformity with a **Verified Integration Status** (no claim of accessing official Pi KYC data). The protocol does not store, request, or claim any raw biometric or KYC data from Pi Network.