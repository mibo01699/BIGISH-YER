...
function testWalletIntegration() {
    ...
    try {
        // 1. اختبار محرك السعر الموحد لـ DEX Pi
        const price = PiYerAMMExchange.getOnChainPrice();
        assert.ok(price > 0, "DEX Pi token liquidity pricing engine failed.");
        console.log(`📊 On-Chain Price Check: 1 Pi = ${price} Tokenized YER.`);

        // 2. اختبار حماية السيولة ومنع الانزلاق السعري (UNICEF Anti-Fraud Rule)
        ...
        console.log("\n🔒 COBRA INTEGRATION SANITY CHECK PASSED SUCCESFULLY!");
        process.exit(0);
    } catch (error) {
