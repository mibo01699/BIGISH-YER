...
// ============================================
// Complete Pi Payment (= Deposit into YER wallet)
// ============================================
    ...
    try {
        ...
        if ((sender.Pi || 0) < pi) {
            return res.status(400).json({
                error: 'رصيد Pi غير كافٍ', current: sender.Pi || 0, required: pi
            });
        }
        if ((sender.YER || 0) < yer) {
            return res.status(400).json({
                error: 'رصيد YER غير كافٍ', current: sender.YER || 0, required: yer
            });
        }
...
app.get('/api/yer/distribution/status', (req, res) => {
    ...
    if (!session || !session.active) {
        return res.json({ success: true, isDistributionActive: false,
...
if (require.main === module) {
    app.listen(PORT, () => {
        console.log(`✅ BIGISH-YER Wallet v3 running on port ${PORT} (${NODE_ENV})`);
    });
}
